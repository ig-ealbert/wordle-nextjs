...
describe("Helper functions for wordle in next.js", () => {
  ...
  it("Validates guess when guess is valid", () => {
    const validatedGuess = validateGuess('abcde');
    assert.strictEqual(validatedGuess, 'abcde');
  });

  it("Validates guess when guess is too short", () => {
    const validatedGuess = validateGuess('abc');
    assert.strictEqual(validatedGuess, 'abcxx');
  });

  it("Validates guess when guess is too long", () => {
    const validatedGuess = validateGuess('abcdefg');
    assert.strictEqual(validatedGuess, 'abcde');
  });
